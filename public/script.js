...
function search(event){
	if(event.which === 13 || event.keyCode === 13){
		...
		if(getCookie("username") != "") setCookie("search", name, 365);
...
function handleResponse(response){
	for (var i=0; i < response.items.length; i++) {
		...
		if(item.volumeInfo.description !== undefined){
			if(item.volumeInfo.description.length > 500){
				content += item.volumeInfo.description.substring(0, 500) + "...";
				content += "<br /><a href=\"#\" id=\"readmore\" title=\"Read More\">Read More</a>";
			}							
		}
		content += '</div></div>';
